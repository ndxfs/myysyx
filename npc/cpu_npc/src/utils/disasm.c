...
void init_disasm() {
  ...
  cs_mode mode = static_cast<cs_mode>(MUXDEF(CONFIG_ISA_x86,      CS_MODE_32,
...
void disassemble(char *str, int size, uint64_t pc, uint8_t *code, int nbyte) {
	...
	//printf("Disasm debug! count=%zu, pc=0x%lx, code_len=%d, code=[0x%02x 0x%02x 0x%02x 0x%02x]\n",
         //count, pc, size, code[0], code[1], code[2], code[3]);
  assert(count == 1);
