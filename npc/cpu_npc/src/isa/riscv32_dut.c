...
bool isa_difftest_checkregs(CPU_state *ref_r, vaddr_t pc) {
	...
	for(int i = idx - 1; i >= 0; i--)
	{
		...
		{
			printf( ANSI_FMT("difftest error", ANSI_FG_RED) ":the reg = " ANSI_FMT("%s", ANSI_FG_RED) " is not equal between DUT and REF. DUT value is " ANSI_FMT("0x%x",ANSI_FG_RED) " while REF value is 0x%x at pc = " ANSI_FMT(FMT_WORD, ANSI_FG_RED)  "\n", reg_name(i), gpr(i), ref_r->gpr[i], pc);
			return false;
		}
	}
	if(ref_r->pc != pc) printf(ANSI_FMT("difftest error", ANSI_FG_RED) "DUT pc is not equal with REF pc at DUT pc = " ANSI_FMT(FMT_WORD, ANSI_FG_RED) ";REF pc =" ANSI_FMT(FMT_WORD, ANSI_FG_RED) "\n", pc, ref_r->pc);
